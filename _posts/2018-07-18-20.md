...
이 내용은 Newyork University의 조경현교수님의 DS-GA 3001강의 lecture note중 Recurrent Neural Networks and Gated Recurrent Units단원을 정리한 내용입니다.
...
$$
\mu = \sigma(\bold{v}^\intercal~g(\bold{W}\phi(x_l)+\bold{U}g(\bold{W}\phi(x_{l-1})+\bold{U}g(\bold{W}\phi(x_{l-2}))+\dotsm)))
$$

여기서 가장 최초의 memory인 $h_0$는 보통 all zero vector를 사용한다. 그리고 $\bold{W, U}$는 shared parameter다.

또 다른 예를 들어 보자. 이번에도 감정 분석 문제이지만, 이번에는 분석해야 할 class가 3개, 즉 positive(1), neutral(2), neagtive(3)으로 분석해야 하는 문제에 대해 살펴보자. 이번에는 출력값이 $\mu$값 1개가 아닌, 3개의 $\mu$를 가지는 $\bold{\mu}= [\mu_1, \mu_2, \mu_3]$벡터를 출력으로 구해야 한다.
구하는 방식은 이전과 거의 비슷하지만 마지막 output을 뽑아낼 때 sigmoid가 아닌 softmax를 사용해야 한다.

$$
\mu = [\mu_1, \mu_2, \mu_3]^\intercal=softmax(\bold{v}h_l)
$$
...
$$
h_t=f(x_t,y_{t-1},h_{t-1})=g(\bold{W}_x\phi_x(x_t)+\bold{W}_y\phi_y(y_{t-1}+\bold{W}_hh_{t-1}))
$$
...
$$
\tilde{h}_t=g(\bold{W}\phi(x_t)+\bold{U}(\bold{r}\odot h_{t-1}))
$$
...
$$
h_t=(1-\bold{u})\odot h_{t-1}+\bold{u}\odot\tilde{h}_t
$$

수식을 보면 아직 소개하지 않은 값들이 있는데 $\bold{r, u}$이다.
...
이제 우리가 알아볼 GRU에 대해서 살펴보자. GRU의 경우에는 위의 CPU가 working하는 과정과 매우 유사하다. 하지만 차이점에 대해서 먼저 생각해보면 우선 GRU의 경우 CPU처럼 instruction들에 대한 정보가 없어서 $\bold{u,r}$를 미리 setting 할 수 없다.
그리고 또 큰 차이점이자 문제점이라 할 수 있는 부분은 $\bold{u,r}$를 binary하게 0,1 값으로만 준다면 학습 시 미분값이 거의 대부분 0이 되서 학습을 할 수 없다.
따라서 두 벡터를 binary한 값이 아닌, [0,1]의 값을 가지는 real valued vector로 만든다.


$$
\bold{u}\in[0,1]^{n_h},~~\bold{r}\in[0,1]^{n_h}
$$
...
$$
\begin{matrix}
\bold{r}_t=\sigma(\bold{W}_r\phi(x_t)+\bold{U}_rh_{t-1})\\
\bold{u}_t=\sigma(\bold{W}_u\phi(x_t)+\bold{U}_u(\bold{r}\odot h_{t-1}))\\
\end{matrix}
$$
...
$$
\begin{matrix}
\tilde{h}=\tanh(\bold{W}x_t+r_t\odot\bold{U}h_{t-1})\\
h_t=\bold{u}_t\odot h_{t-1}+(1-\bold{u}_t)\odot\tilde{h}_t
\end{matrix}
$$

전체적인 구조와 각 계산들에 대해 하나씩 그림으로 보겠습니다. 그림의 출처는 [이곳](https://towardsdatascience.com/understanding-gru-networks-2ef37df6c9be)이며 소개한 수식에서 $\bold{u}$가 $\bold{v}$로 표현됬다는 차이가 있습니다.
...
$$
f(x_t,\bold{h}_{t-1}=g(\bold{w}\phi(x_t)+\bold{U}\bold{h}_{t-1}))
$$
...
이 식에서 의미하는 것은 $\bold{U}$의 eigenvalue의 최대값이 $\frac1\lambda$보다 크다면 미분값이 무한대로 갈 것이고, 반대의 경우에는 0으로 수렴한다는 내용이다.
