...
여기서 $$\sigma$$는 logistic sigmoid 함수이고 $$[.]_j$$는 벡터의 $$j$$번째 우너소를 뜻한다. 그리고 $$\mathbf{x}$$ 와 $$\mathbf{h}_{t-1}$$은 각각 input과 이전 hidden state를 뜻한다. 마지막으로 $$\mathbf{W}_r$$과 $$\mathbf{U}_r$$은 학습해야 할 weight matrix이다.
